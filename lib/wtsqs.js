...
class WTSQSError extends Error {
  constructor (...args) {
    super(...args)
    Error.captureStackTrace(this, WTSQSError)
  }
}
...
class WTSQS {
  ...
  constructor ({
    ...
    if (url === undefined) throw new WTSQSError('url required')
    if (accessKeyId === undefined) throw new WTSQSError('accessKeyId required')
    if (secretAccessKey === undefined) throw new WTSQSError('secretAccessKey required')
  ...
  async enqueueOne (payload, { messageGroupId } = {}, sqsOptions = {}) {
    if (payload === undefined) throw WTSQSError('payload is required')
  ...
  async enqueueMany (payloads, { messageGroupId } = {}, sqsOptions = {}) {
    if (!(payloads instanceof Array)) throw WTSQSError('payloads must be of type array')
